...
TGraph* HistogramManager::book(std::string GraphName,std::string xlabel, std::string ylabel,int nEntries){
    /** use the Int_n constructor of TGraph -> so we only need the number of entries in the TGraph **/
    TGraph* tmp = new TGraph(nEntries); 
    tmp->SetName(GraphName.c_str());
    tmp->SetTitle(GraphName.c_str());
    tmp->GetXaxis()->SetTitle(xlabel.c_str());
    tmp->GetYaxis()->SetTitle(ylabel.c_str());

    this->record(tmp); // pass ownership of the pointer object to the HistogramManager class so it stays in scope for the final usage
    return tmp; 
}
...
void HistogramManager::record(TGraph* mygraph){
    m_allGraphs.push_back(mygraph);
}
