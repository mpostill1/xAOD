...
EL::StatusCode ClusterHistsAlgo :: initialize ()
{
  ...
  vec_combined_total.clear();
  vec_combined_total.shrink_to_fit();
  vec_eventweight_total.clear();
  vec_eventweight_total.shrink_to_fit();
...
EL::StatusCode ClusterHistsAlgo :: execute ()
{
  ...
  double rho_orig = 0;
  const xAOD::EventShape* rho_orginal(nullptr);
  ANA_CHECK( HelperFunctions::retrieve(rho_orginal,"Kt4EMTopoOriginEventShape", m_event, m_store, msg()) );
  rho_orig = rho_orginal->getDensity(xAOD::EventShape_v1::EventDensityID::Density);
  ...
  const xAOD::CaloClusterContainer* Cluster_container=nullptr;
  ...
  ANA_CHECK( HelperFunctions::retrieve(Cluster_container,  "CaloCalTopoClusters", m_event, m_store, msg()) );
  ANA_CHECK( m_plots->execute( Cluster_container, eventWeight ));
  //ANA_CHECK( HelperFunctions::retrieve(CFE_container,  "GlobalChargedParticleFlowObjects", m_event, m_store, msg()) );
  //ANA_CHECK( HelperFunctions::retrieve(NFE_container,  "GlobalNeutralParticleFlowObjects", m_event, m_store, msg()) );
  ...
  //std::unique_ptr<const xAOD::AuxContainerBase> central_combinedAux= std::make_unique<xAOD::AuxContainerBase>();
    ...
    // Copy this NFE to the output container:    // xAOD::FlowElement* NFE= new xAOD::FlowElement();
 ...
 for (const xAOD::CaloCluster* CFE_Element : *Cluster_container) {
    // Copy this CFE to the output container:
    xAOD::CaloCluster* CFE = new xAOD::CaloCluster();
    central_fwd_combined->push_back(CFE); // CFE acquires the central_combined auxstore
    *CFE=*CFE_Element; // copies auxdata from one auxstore to the other
  }
  ...
  //ANA_CHECK( m_plots->execute( rho_orginal, eventWeight )); 
  ...
  if(rho_orig!=0){
    std::vector<std::vector<double>> vec_normalised=SlidingWindow::SlidingWindowExecuteBoth_Normalised(AntiKt4EMPFlowJets,central_fwd_combined.get(),rho_orig);
    vec_normalised_total.push_back(vec_normalised);}
  //std::vector<std::vector<double>> vec_averaged=SlidingWindow::SlidingWindowExecuteBoth(AntiKt4EMPFlowJets, central_fwd_combined.get());
  ...
  ANA_CHECK(m_plots->RhoHistFill_orig(eventWeight,rho_orig) );
  vec_combined_total.push_back(vec_combined);
  vec_eventweight_total.push_back(eventWeight);
...
EL::StatusCode ClusterHistsAlgo :: finalize () {
  m_plots->RhoEtaHistFill_combined_containers_average(vec_combined_total,vec_eventweight_total);
  
  m_plots->RhoEtaHistFill_combined_containers_average_comparsion(vec_normalised_total,vec_eventweight_total);
  return EL::StatusCode::SUCCESS; 
}
