...
#include "xAODAnaHelpers/fastjetmp.h"
std::vector<std::vector<double>> SlidingWindow::SlidingWindowExecuteBoth_Normalised(const xAOD::JetContainer* jetcont, const xAOD::IParticleContainer* Iparcont,double rho_orig){
...
//setup variables
 ...
 for ( const auto* cft : *Iparcont) {
   if ( std::abs(cft->rapidity()) > rapmax ){
...
//clear this vector if there's some junk in memory
 ...
 for (const xAOD::Jet* pJet :*jetcont ) {
   ...
   std::cout<<"rho_ratio "<<rho/rho_orig<<std::endl;
